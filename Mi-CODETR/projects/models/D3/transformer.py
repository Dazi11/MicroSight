...
class CoDinoTransformer(CoDeformableDetrTransformer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.LSTMConvCombined = nn.Conv1d(self.embed_dims * 2, self.embed_dims, 1)
        ...
        self.Lup1 = nn.ConvTranspose2d(self.embed_dims, self.embed_dims, 4, 2, 1)
        self.Lup2 = nn.ConvTranspose2d(self.embed_dims, self.embed_dims, 4, 2, 1)
        self.Lup3 = nn.ConvTranspose2d(self.embed_dims, self.embed_dims, 4, 2, 1)
        self.Lup4 = nn.ConvTranspose2d(self.embed_dims, self.embed_dims, 4, 2, 1)
        self.Lact = nn.GELU()
    ...
    def forward(self,
        ...
        lstm_flatten = []
        # lstmoutput -> B C H W
        lstmOutput = lstm_feature.reshape(first_h, first_w, first_bs, first_c).permute(2, 3, 0, 1)
        x1 = self.Lact(self.Lup1(lstmOutput))[:, :, :second_h, :second_w]
        x2 = self.Lact(self.Lup2(x1))
        x3 = self.Lact(self.Lup3(x2))
        x4 = self.Lact(self.Lup4(x3))
        lstm_flatten.append(x4.flatten(2).transpose(1, 2))
        lstm_flatten.append(x3.flatten(2).transpose(1, 2))
        lstm_flatten.append(x2.flatten(2).transpose(1, 2))
        lstm_flatten.append(x1.flatten(2).transpose(1, 2))
        lstm_flatten.append(lstmOutput.flatten(2).transpose(1, 2))
        lstm_flatten = torch.cat(lstm_flatten, 1)
        # lstm_flatten -> B h*W C

        pre_feature = feat_flatten
        feat_flatten = self.LSTMConvCombined(
            torch.cat([lstm_flatten, pre_feature.transpose(0, 1)], dim=2).permute(0, 2, 1)).permute(2, 0, 1)
        # q = k = feat_flatten
