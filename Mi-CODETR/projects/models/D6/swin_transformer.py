...
class PartAttention(nn.Module):
    def __init__(self, num_patches=196, embed_dim=768):
        ...
        self.layerNorm2 = nn.LayerNorm(num_patches)
    ...
    def forward(self, x):
        ...
        similarityMatrix = self.layerNorm2(similarityMatrix)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.haslstm = haslstm

        if haslstm:
            window_sizeInt = window_size[0]
            self.sizeInt = window_sizeInt * window_sizeInt
            self.lstm = LSTM(self.sizeInt, self.sizeInt, 2, True)
            self.lstmNorm = nn.LayerNorm(self.sizeInt)
            self.MGCNNet = MGCNNet(input_dim=dim, output_dim=dim)
            self.masked_pos = nn.Parameter(torch.Tensor(self.sizeInt, self.sizeInt))
            self.simi = PartAttention(self.sizeInt, dim)
            self.Conv1 = nn.Conv2d(2, 8, 1, 1)
            self.Conv2 = nn.Conv2d(8, 1, 1, 1)
            self.ConvCombined = nn.Conv1d(dim * 2, dim, 1)
    ...
    def forward(self, x, mask=None):
        ...
        B_, N, C = x.shape
        pre_qkv = self.qkv(x)
        qkv = pre_qkv.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        # q,k,v ->[B_,heads,N,C/heads]
        ...
        if self.haslstm:
            ...
            heads = attn.shape[1]
            # contri -> [B*heads,N,N]
            contri = self.lstmNorm(self.lstm(attn.reshape(B * heads, self.sizeInt, self.sizeInt)))
            similarity = self.simi(x)
            attn_contri = contri.reshape(B, heads, self.sizeInt, self.sizeInt).mean(dim=1)
            adjacency = torch.cat([similarity.unsqueeze(1), attn_contri.unsqueeze(1)], dim=1)
            # adjacency = similarityMatrix.squeeze(1)
            adjacency = self.Conv2(self.Conv1(adjacency)).squeeze(1)
            partOutput = self.MGCNNet(adjacency=adjacency, x=x, masked_pos=self.masked_pos)
            attn = contri.reshape(B, heads, self.sizeInt, self.sizeInt)
        ...
        if self.haslstm:
            x_pre = x
            x = self.ConvCombined(torch.cat([x_pre.permute(0, 2, 1), partOutput.permute(0, 2, 1)], dim=1)).permute(0, 2,
                                                                                                                   1)
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, haslstm=False):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, haslstm=haslstm)
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                haslstm=True if (i_layer == self.num_layers - 1) else False)
